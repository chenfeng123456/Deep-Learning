...
class DateOffset(BaseOffset):
    ...
    def onOffset(self, dt):
        if self.normalize and not _is_normalized(dt):
            return False
        # XXX, see #1395
        if isinstance(self, DateOffset) or isinstance(self, Tick):
            return True
...
class SemiMonthOffset(DateOffset):
    ...
    def apply(self, other):
        ...
        # initially positive.
        if isinstance(self, SemiMonthBegin) and (
                self.n <= 0 and other.day == 1):
            n -= 1
        elif isinstance(self, SemiMonthEnd) and (self.n > 0 and
